...
serve(async (req) => {
  ...
  try {
    ...
    // Call OpenAI API with the image as input
...
Ensure a full-body view is clearly visible, showing from head to toe.`
    ...
    // Extract the DALL-E prompt from GPT-4o's text response
    const gpt4oResponse = openAIData.choices[0].message.content
    console.log('GPT-4o response:', gpt4oResponse)
    
    // Now use DALL-E to generate the avatar based on GPT-4o's instructions
    const dalleResponse = await fetch('https://api.openai.com/v1/images/generations', {
      ...
      body: JSON.stringify({
        model: "dall-e-3",
        prompt: `Based on this exact person in the reference photo I just shared with you, please generate a realistic, full-length image preserving their exact appearance completely. Do NOT alter their facial features, skin color, body shape, or overall identity at all.
...
Ensure a full-body view is clearly visible, showing from head to toe.`,
    ...
    const generatedAvatarUrl = dalleData.data[0].url
    
    console.log('Avatar generated by DALL-E:', generatedAvatarUrl)
